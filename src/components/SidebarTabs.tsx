        ...
        {activeTab === 'news' ? (
          <>
            ...
            <div className="px-4 py-2 border-t border-neutral-800 flex items-center justify-between">
              <span className="text-neutral-600 text-[9px]">
                Updated: {mounted && lastRefresh ? lastRefresh.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }) : '—'}
              </span>
          ...
          <>
            ...
            {visibleActivityCount < activities.length && (
              <button
                ...
                className="w-full px-4 py-2 text-[10px] text-neutral-400 hover:text-white hover:bg-neutral-900 transition-colors border-t border-neutral-800"
              >
                Show more ({activities.length - visibleActivityCount} remaining)
              </button>
            )}
          </>
        )}
